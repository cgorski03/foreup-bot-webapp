...
export const useGetCourses = () => {
  const { commonFetch, isLoading, data } = useFetch<GolfCourse[]>({
    endpoint: "/courses",
    method: "GET",
  });
  ...
  return { getCourses, isLoading, data }
};

export const useCreateSearch = () => {
  const { commonFetch, isLoading, data } = useFetch<string>({
    endpoint: "/search",
    method: "POST",
  }) 
  const createSearch = (input: CreateSearchInput ) => commonFetch({ input });
  
  return { createSearch, isLoading, data }
}